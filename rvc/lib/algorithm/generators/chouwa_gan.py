...
class InterpolateConv1d(nn.Module):
    """
    Interpolation + Conv1d upsampling block.
    Avoids the strong spectral images created by ConvTranspose1d's zero-insertion.
    Used in modern vocoders (SoundStream, Encodec) for low upsampling rates.
    """
...
class HiFiGANNSFHead(nn.Module):
    ...
    def __init__(
        ...
        self.har_prefilters = nn.ModuleList()
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            ...
            if u <= 2:
                # Use interpolation + Conv1d for low upsampling rates to avoid
                # spectral images ("mirroring") created by ConvTranspose1d
                self.ups.append(
                    InterpolateConv1d(
                ...
                # Interpolation doesn't create the severe spectral images that
                # ConvTranspose1d does. An anti-alias filter here is unnecessary
                # and can actually attenuate desired high frequencies.
                self.anti_alias_convs.append(nn.Identity())
            else:
                # Use ConvTranspose1d for high upsampling rates where it's more efficient
                self.ups.append(
                ...
                # Frozen sinc filter to aggressively clean up the strong
                # spectral images generated by high-ratio ConvTranspose1d
                aa_kernel = max(9, u * 2 + 1)
                aa_cutoff = 0.5 / u
            ...
            # Anti-alias pre-filter for PCPH source before strided har_conv.
            # Without this, harmonics above Nyquist/(stride) fold back during
            # decimation, creating visible spectral line artifacts.
            if stride > 1:
                pre_kernel = stride * 4 + 1  # Generous kernel for clean cutoff
                pre_cutoff = 0.5 / stride
                self.har_prefilters.append(
                    _make_sinc_lowpass(1, pre_kernel, pre_cutoff, freeze=True)
                )
            else:
                self.har_prefilters.append(nn.Identity())
    ...
    def forward(self, x: torch.Tensor, har_source: torch.Tensor,
        ...
        for i in range(self.num_upsamples):
            ...
            har_out = self.har_convs[i](self.har_prefilters[i](har_source))
        ...
        # Ensure exact output length. InterpolateConv1d lacks the output_padding
        # parameter of ConvTranspose1d, which can lead to a 1-2 sample difference
        # at the end of the generator chain, breaking STFT loss calculations.
        expected_len = har_source.shape[-1]
